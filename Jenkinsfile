pipeline {
  ...
  stages {
    ...
    stage('Test'){
        steps {
            echo "Testing"
            sh 'mkdir Terradue.OpenSearch.RdfEO.Test/out'
    ...
    stage('Package') {
      steps {
            ...
            sh "nuget4mono -g origin/${env.BRANCH_NAME} -p ${workspace}/Terradue.OpenSearch.RdfEO/packages.config ${workspace}/Terradue.OpenSearch.RdfEO/bin/Terradue.OpenSearch.RdfEO.dll"
            sh 'cat *.nuspec'
